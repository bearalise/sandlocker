...
| **M3 Beta**（v0.5~0.8） | T+28 ~ T+40 周 | 集群编排；API Key/配额/审计；可观测性。（OCI Runtime 独立形态、Helm Chart、Go SDK 移至 GA 后/社区轨道，见砍单预案） | 3 节点集群 SLO 达标；外部 5 个团队试用反馈闭环；**闸门：确认 GA 范围；启动安全审计采购（排期通常 8–10 周）** |
...
> | 2 | 控制台 | 移出 V1 范围（4.1 图注） | 无任何里程碑认领；CLI 够用，留待社区或商业化阶段 |
